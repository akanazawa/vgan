...
class Discriminator(nn.Module):
    def __init__(self, z_dim, nlabels, size, embed_size=256, nfilter=64, nfilter_max=1024, reg_type='none', **kwargs):
        super().__init__()
        self.embed_size = embed_size
        self.reg_type = reg_type
        self.vgan_latent_dim = kwargs['vgan_latent_dim']
        s0 = self.s0 = 4
        nf = self.nf = nfilter
        nf_max = self.nf_max = nfilter_max
        ...
        elif 'instnoise' in self.reg_type:
            # Use the same final dim as vgan
            self.mean_fn = nn.Conv2d(self.nf0, self.vgan_latent_dim, 1)
            self.fc = nn.Linear(self.vgan_latent_dim*s0*s0, nlabels)
        else:
    ...
    def forward(self, x, y):
        ...
        if 'instnoise' in self.reg_type:
            # print('adding instance noise!!')
